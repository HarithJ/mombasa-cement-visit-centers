...
try{
 ...
 for(const mount of ['',prefix]){
  ...
  for(const version of ['v1','v2','v3']){
   ...
   assert.match(await page.locator('#route-image').getAttribute('src'),/routes\/galana\.png$/);
   await page.locator('.close-button').click();
   await page.locator('[data-book="feeding"]').click();
   await page.locator('#inline-map summary').click();
   assert.match(await page.locator('#route-image').getAttribute('src'),/routes\/feeding\.png$/);
   await page.locator('.close-button').click();
   await page.locator('[data-book="galana"]').click();
