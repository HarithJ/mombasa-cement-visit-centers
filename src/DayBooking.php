...
final class DayBooking {
    public static function validate(array $input, array $destinations): array {
        $data = [];
        foreach (['fullName', 'phone', 'location', 'visitDate', 'timeSlot', 'attendees', 'overnight', 'arrivalDate', 'departureDate', 'overnightGuests'] as $key) $data[$key] = is_string($input[$key] ?? null) ? trim($input[$key]) : '';
        $errors = [];
