            ...
            <div id="form-view" <?= $confirmation ? 'hidden' : '' ?>>
                ...
                <form id="booking-form" method="post" action="<?= e($bookingPath) ?>" novalidate>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="field full"><label for="location">Where would you like to visit? <span aria-hidden="true">*</span></label><select id="location" name="location" required aria-describedby="location-error"><?php foreach ($destinations as $id => $destination): ?><option value="<?= e($id) ?>" data-map-url="<?= e($mapLinks[$id]) ?>" <?= ($input['location'] ?? 'sahajanand') === $id ? 'selected' : '' ?>><?= e($destination['name']) ?></option><?php endforeach; ?></select><span class="field-error" id="location-error"><?= e($errors['location'] ?? '') ?></span></div>
                        <div class="destination-location full">
                            <span class="location-caption">FIND YOUR WAY</span>
