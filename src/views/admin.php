...
<section class="panel"><h2>Visitor feedback</h2><?php if (!$booking['attendance']): ?><div class="empty-feedback"><span class="badge pending">Awaiting feedback</span><p>No feedback received.</p></div><?php else: ?><dl><dt>Self-reported attendance</dt><dd><span class="badge"><?=$booking['attendance']==='attended'?'Attended':'Did not attend'?></span></dd><?php if($booking['attendance']==='attended'): ?><dt>Rating</dt><dd class="rating"><?=ae($booking['rating'])?> / 5</dd><?php endif; ?>
...
<div class="table" role="region" aria-label="Bookings"><table role="table"><thead><tr><th>Reference</th><th>Visitor</th><th>Destination</th><th>Visit date</th><th>Time</th><th>Guests</th><th>Feedback</th></tr></thead><tbody>
...
<td role="cell" data-label="Feedback"><span class="badge <?=$row['has_feedback']?'':'pending'?>"><?=$row['feedback_attendance']==='attended'?'Attended':($row['feedback_attendance']==='not_attended'?'Did not attend':'Awaiting feedback')?></span></td>
</tr><?php endforeach; ?>
