...
<head>
    ...
    <link rel="stylesheet" href="/assets/versions/v2/travel.css">
    ...
    <script src="/assets/versions/v2/app.js" defer></script>
</head>
<body>
        ...
        <div class="destinations wrap" id="destinations" aria-label="Choose a destination">
            ...
            <article class="destination" data-destination="<?= e($id) ?>">
                ...
                <div class="destination-info">
                    ...
                    <a class="book-button" data-book="<?= e($id) ?>" href="<?= e($bookingPath) ?>?book=<?= e($id) ?>">Plan this visit <?= icon('arrow') ?></a>
                </div>
            </article>
            <?php endforeach; ?>
        </div>
...
<dialog id="booking-dialog" aria-labelledby="<?= $confirmation ? 'confirmation-title' : 'booking-title' ?>" <?= $openForm || $confirmation ? 'open' : '' ?>>
    <div class="dialog-toolbar"><span class="itinerary-label">VISIT NYUMBA <span>/ YOUR VISIT PLAN</span></span><a role="button" href="<?= e($bookingPath) ?>" class="close-button" aria-label="Close booking form"><?= icon('close') ?></a></div>
    <div class="dialog-shell">
        <aside class="booking-aside"><img id="booking-image" src="/assets/galana-placeholder.svg" alt="" width="500" height="800"><div class="aside-overlay"><p class="eyebrow">YOUR NEXT VISIT</p><h2 id="aside-destination"><?= e($destinations[$confirmation['destination'] ?? $input['location'] ?? 'sahajanand']['name'] ?? $destinations['sahajanand']['name']) ?></h2><p>A little time.<br>A different perspective.</p><span><?= icon('leaf') ?> Visit Nyumba</span></div></aside>
            ...
            <div id="form-view" <?= $confirmation ? 'hidden' : '' ?>>
                ...
                <form id="booking-form" method="post" action="<?= e($bookingPath) ?>" novalidate>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="field"><label for="email">Email address (optional)</label><input id="email" name="email" value="<?= e($input['email'] ?? '') ?>" type="email" autocomplete="email" maxlength="254" placeholder="e.g. alex@example.com" aria-describedby="email-error"><span class="field-error" id="email-error"><?= e($errors['email'] ?? '') ?></span></div>
                        <input id="location" name="location" type="hidden" value="<?= e($input['location'] ?? 'sahajanand') ?>">
                        <?php foreach ($destinations as $id => $destination): ?><span hidden data-location-meta="<?= e($id) ?>" data-map-url="<?= e($mapLinks[$id]) ?>" data-route-image="<?= e($routeIllustrations[$id]['image']) ?>" data-route-alt="<?= e($routeIllustrations[$id]['alt']) ?>" data-name="<?= e($destination['name']) ?>"></span><?php endforeach; ?>
                        <span class="field-error" id="location-error" hidden><?= e($errors['location'] ?? '') ?></span>
                        ...
                        <div class="field"><label for="time-slot">Time slot <span aria-hidden="true">*</span></label><select id="time-slot" name="timeSlot" required aria-describedby="time-slot-error schedule-note"><option value="">Select a time</option><?php foreach ($schedule[$input['location'] ?? 'sahajanand']['slots'] ?? [] as $time): ?><option value="<?= e($time) ?>" <?= ($input['timeSlot'] ?? '') === $time ? 'selected' : '' ?>><?= e(date('g:i a', strtotime($time))) ?></option><?php endforeach; ?></select><span class="field-error" id="time-slot-error"><?= e($errors['timeSlot'] ?? '') ?></span></div>
                    ...
                    <label class="checkbox-label"><input id="transport-requested" name="transportRequested" type="checkbox" aria-labelledby="transport-label" aria-describedby="transport-hint transport-requested-error" <?= ($input['transportRequested'] ?? '') === 'on' ? 'checked' : '' ?>><span class="checkbox-copy"><strong id="transport-label">I would like the company to arrange transport for my visit.</strong><span id="transport-hint">Transport is subject to availability. Our team will contact you to confirm arrangements.</span></span></label>
                    <span class="field-error" id="transport-requested-error"><?= e($errors['transportRequested'] ?? '') ?></span>
                    ...
                    <p class="privacy-note">Your name, email address, phone number and visit details are stored to record your visit. Booking emails may be sent to you and the destination team to coordinate your visit.</p>
                        <div class="destination-location">
                            <details id="inline-map">
                                <summary>View illustrated route <span aria-hidden="true">+</span></summary>
                                <?php $route = $routeIllustrations[$input['location'] ?? 'sahajanand'] ?? $routeIllustrations['sahajanand']; ?>
                                <div id="inline-map-frame" class="inline-map-frame"><a id="route-image-link" href="<?= e($route['image']) ?>" target="_blank" rel="noopener noreferrer" aria-label="Open full-size route illustration in a new tab"><img id="route-image" src="<?= e($route['image']) ?>" alt="<?= e($route['alt']) ?>" width="1774" height="887" loading="lazy"></a><span class="route-image-hint">Illustrated route · Not to scale. Tap the image to enlarge.</span></div>
                            </details>
